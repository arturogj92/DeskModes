    ...
    private func loadFromConfigStore() {
        ...
        availableModes = config.modes.map { modeConfig in
            Mode(
                ...
                allowList: modeConfig.allowList.map { AppIdentifier(bundleId: $0.bundleId, displayName: $0.name) },
                appsToOpen: modeConfig.appsToOpen.map { AppIdentifier(bundleId: $0.bundleId, displayName: $0.name) }
            )
        }

        // Convert global allow list
